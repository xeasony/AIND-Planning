...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # check if action's pre-conditions satisfy previous state level.
        # Then build the links between new action node and previous state node.
        for action in self.all_actions:
            ...
            if action_node.prenodes.issubset(self.s_levels[level]):
                ...
                for pre_node in action_node.prenodes:
                    pre_node.children.add(action_node)
                    action_node.parents.add(pre_node)
    ...
    def add_literal_level(self, level):
        ...
        # With all current level's action nodes
        # based on the effect that action produce create new state nodes
        # build the links between new state nodes and current action node.
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Test if two action node are mutually exclusive with each other or not
        ...
        for fluent in a1_eff_add:
            if fluent in a2_eff_rem:
                return True

        for fluent in a1_eff_rem:
            if fluent in a2_eff_add:
                return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for fluent in a1_eff_add:
            if fluent in a2_pre_neg:
                return True

        for fluent in a1_eff_rem:
            if fluent in a2_pre_pos:
                return True

        for fluent in a2_eff_add:
            if fluent in a1_pre_neg:
                return True

        for fluent in a2_eff_rem:
            if fluent in a1_pre_pos:
                return True
    ...
    def h_levelsum(self) -> int:
        ...
        # Calculate heuristic score by finding smallest level of each goals
        for sub_goal in self.problem.goal:
            found = False
            sub_goal_node = PgNode_s(sub_goal, True)

            for level in range(0, len(self.s_levels)):
                if sub_goal_node in self.s_levels[level]:
                    found = True
                    level_sum += level
                    break
